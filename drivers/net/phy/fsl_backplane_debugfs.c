...
void backplane_dbg_add(struct phy_device *phydev)
{
	...
	if (!phydev->attached_dev) {
		pr_err("FSL_BACKPLANE: backplane_dbg_add failed: no phydev->attached_dev \n");
		return;
	}
	...
	phydev_dbg_root = debugfs_create_dir(netdev_name(phydev->attached_dev),
...
void backplane_dbg_init(void)
{
	...
	if (!fsl_backplane_dbg_root) {
		pr_err("FSL_BACKPLANE: debugfs create failed\n");
		return;
	}

	pr_info("FSL_BACKPLANE: debugfs created\n");
}
